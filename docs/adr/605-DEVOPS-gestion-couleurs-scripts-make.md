...
classification:
  ...
  tech_areas:
    ...
    - "bash"
    - "terminal"
tags: ["makefile", "colors", "ansi", "printf", "terminal", "devops"]
...
# ADR-605 : Gestion des couleurs dans les scripts Make
...
- **CI/CD** : Logs colorés dans GitHub Actions

### Négatives
- **Syntaxe** : Nécessite `\n` explicite à la fin de chaque message
- **Migration** : Requiert de modifier les `@echo` existants si présents
...
### Fichiers Concernés
- `Makefile` — Makefile racine du projet private-llm-gateway (à créer/adapter)
...
| Date | Action | Auteur |
|------|--------|--------|
| 2026-06-05 | Création ADR-605 | @dev-team |
